...
import { FaInstagram, FaFacebookF, FaTwitter, FaImages, FaTrash } from 'react-icons/fa';
...
const FinalDetails = ({ formData, updateFormData, onBack, setToast }: FinalDetailsProps) => {
  ...
  const handleSubmit = async () => {
    try {
      ...
      const event = {
        ...
        ticketType: formData.ticketType.map(ticket => ({
      ...
      <div>
        ...
        <div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 gap-4 mb-4">
            {formData.gallery.map((_, index) => (
              <div key={index} className="relative aspect-video bg-gray-100 dark:bg-gray-700 rounded-lg">
        ...
        <div className="flex justify-between mt-8">
          ...
          <button
            ...
            className={`px-6 py-3 bg-gradient-to-r from-blue-600 to-purple-600 text-white rounded-lg transform transition-all duration-200 shadow-lg ${isSubmitting ? 'opacity-75 cursor-not-allowed' : 'hover:from-blue-700 hover:to-purple-700 hover:scale-105 hover:shadow-xl'}`}
          >
